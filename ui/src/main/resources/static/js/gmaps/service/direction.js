		...
		function($q) {
			...
			var getPath = function(markers, mymap) {
				...
				if (markers && markers.length == 2) {
					...
					/*
					 * var rendererOptions = { map: mymap } var
					...
					directionsService.route(
									...
									function(response, status) {
										if (status == google.maps.DirectionsStatus.OK) {
											...
											deferred.resolve({
												encpath:getEncodedPath(response),
												duration:getDuration(response.routes[0].legs[0]),
												distance:getDistance(response.routes[0].legs[0]),
											})
										} else {
											console
													.log('Direction service of google  failed due to: '
															+ status);
			...
			function getDuration(data) {
				...
				for (var i = 0; i < mylegs.steps.length; i++) {
					dur = dur + parseInt(mylegs.steps[i].duration.value);
				}
			...
			function getDistance(data) {
				...
				var dis = 0;
				for (var i = 0; i < mylegs.steps.length; i++) {
					dis = dis + parseInt(mylegs.steps[i].distance.value);
				}
				console.log("duration: " + dis + "m");
				return dis;
			}
			/* Show Steps and attach instructions */
			function showSteps(directionResult, mymap) {
				// For each step, place a marker, and add the text to
				// the marker's
				// info window. Also attach the marker to an array so we
				// can keep track of it and remove it when calculating
				// new
				// routes.
				// For Icon path go to github rep of google material
				// Icons and show it in RAW.
				...
				for (var i = 0; i < myRoute.steps.length; i++) {
					...
					attachInstructionText(marker,
							myRoute.steps[i].instructions, mymap);
			...
			function attachInstructionText(marker, text, mymap) {
				google.maps.event.addListener(marker, 'click',
						function() {
							...
							// Open an info window when the marker is
							// clicked on,
			...
			function getEncodedPath(directionResult) {
				...
				var encodeString = google.maps.geometry.encoding
						.encodePath(mypath);
